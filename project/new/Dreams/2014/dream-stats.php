...
foreach ($notes as $note) {
	...
	$note_array[date("F j Y", $createdDate)] = array('title' => $note->title, 'tags' => $tags, 'guid' => $note->guid);
	#echo date("F j Y", $createdDate)."<br>";
	if(isset($month_count[date("y/m", $createdDate)])){
		$month_count[date("y/m", $createdDate)] += 1;
	} else{
		$month_count[date("y/m", $createdDate)] = 1;
	}
...
<p align="center"><a href="#today" class="btn btn-success">jump to now</a> <a href="map.html" class="btn btn-success">map it!</a></p>
<table>
<?php
	$start = 1350838800;
	...
	for ($dater = $start; $dater < $end; $dater += 86400){
		...
		echo date("j", $dater).'</p>';
		if(strpos($note_array[$thisDate]['tags'],"sex") !== FALSE || strpos($note_array[$thisDate]['tags'],"private") !== FALSE){
			echo '<p class="t">Private</p>';
		}else{
			echo '<p class="t"><a href="dream.php?guid=' . $note_array[$thisDate]['guid'] . '">' . $note_array[$thisDate]['title'] . '</a></p>';
			echo '<p class="g">' . $note_array[$thisDate]['tags'];
		}
		echo '</p></td>';
